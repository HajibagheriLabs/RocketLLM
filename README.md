...
| `shard_handle_limit` | **profile** measured commit headroom | How many shard files may stay memory-mapped at once: `auto`, `unbounded`, or a count. A mapping is charged against the OS commit limit on Windows and against nothing on Linux, so a checkpoint larger than the measured headroom is read through a small pool of handles instead of keeping them all open. Changes only what stays *open*, never what is read. |
