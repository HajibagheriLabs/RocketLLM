...
| `shard_handle_limit` | **profile** measured commit headroom | How much of a checkpoint may stay memory-mapped: `auto`, `unbounded`, `direct`, or a count. Mapping is the cheaper read � safetensors returns tensors that alias the file instead of copying out of it � but a mapping is charged against the OS commit limit on Windows (and against nothing on Linux) for as long as any tensor read from it lives. `auto` maps where the measured headroom covers the checkpoint and reads byte ranges where it does not. Both routes return the same bytes. |
