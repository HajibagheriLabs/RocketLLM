...
| How a shard is read | `caps.commit_headroom()` � the OS's own commit accounting, then psutil, then plain host RAM � against the checkpoint's own size | Byte-range reads instead of memory mappings | Same bytes, one copy more. safetensors returns tensors that ALIAS the mapping, so on an OS that charges mappings against a commit limit one cached 4MB expert holds its whole 1.57GB shard � a 67GB model exhausted an 18GB limit that way, and closing handles could not release what live tensors still pointed at. Linux under default overcommit charges nothing for a read-only mapping and keeps the zero-copy route. A big-endian host cannot read the payload as stored, so it maps regardless and bounds how many shards stay open. |
