...
_ROCKETLLM_DIR = Path(__file__).resolve().parents[1] / 'rocketllm'

# Bind the package without running rocketllm/__init__.py: that module pulls in the MLX backend on
# macOS and the full transformers stack elsewhere, none of which these splitter tests need.
if 'rocketllm' not in sys.modules:
    _pkg = types.ModuleType('rocketllm')
    _pkg.__path__ = [str(_ROCKETLLM_DIR)]
    sys.modules['rocketllm'] = _pkg

from rocketllm.persist import model_persister as _persister_mod
from rocketllm.persist.safetensor_model_persister import SafetensorModelPersister
...
from rocketllm.utils import split_and_save_layers
